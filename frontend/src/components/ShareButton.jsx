import { useState, useRef, useEffect } from 'react';
import { createPortal } from 'react-dom';
import { getAyahShareImageUrl } from '../api/client';

/**
 * ShareButton - Component for sharing ayahs as beautiful images
 */
function ShareButton({
    surahId,
    ayahNumber,
    surahName,
    surahEnglishName,
    surahTranslation,
    totalAyahs,
    translation = 'en.sahih'
}) {
    const [isOpen, setIsOpen] = useState(false);
    const [imageUrl, setImageUrl] = useState('');
    const [imageLoading, setImageLoading] = useState(false);
    const [copied, setCopied] = useState(false);
    const [imageFormat, setImageFormat] = useState('landscape'); // 'landscape', 'square', 'portrait'
    const [imageStyle, setImageStyle] = useState('classic');
    const modalRef = useRef(null);

    // Get base URL for sharing - use production URL if set, otherwise use current origin
    const productionUrl = import.meta.env.VITE_PUBLIC_URL || 'https://quran.hyperflash.uk';
    const baseUrl = productionUrl || window.location.origin;
    const ayahUrl = `${baseUrl}/quran/${surahId}#ayah-${ayahNumber}`;

    // Generate image URL when modal opens or format changes
    useEffect(() => {
        if (isOpen) {
            const timestamp = Date.now();
            const isSquare = imageFormat === 'square';
            const isPortrait = imageFormat === 'portrait';
            const url = getAyahShareImageUrl(surahId, ayahNumber, translation, isSquare, isPortrait, imageStyle, 'png') + `&t=${timestamp}`;
            setImageUrl(url);

            // Preload the image
            setImageLoading(true);
            const img = new Image();
            img.onload = () => setImageLoading(false);
            img.onerror = () => setImageLoading(false);
            img.src = url;
        }
    }, [isOpen, surahId, ayahNumber, translation, imageFormat, imageStyle]);

    // Close modal on click outside
    useEffect(() => {
        const handleClickOutside = (event) => {
            if (modalRef.current && !modalRef.current.contains(event.target)) {
                setIsOpen(false);
            }
        };

        if (isOpen) {
            document.addEventListener('mousedown', handleClickOutside);
            return () => document.removeEventListener('mousedown', handleClickOutside);
        }
    }, [isOpen]);

    // Native share with image support (works on mobile and supported browsers)
    const nativeShare = async () => {
        const fullSurahName = surahEnglishName && surahTranslation
            ? `${surahEnglishName} (${surahTranslation})`
            : surahEnglishName || surahName;
        const ayahInfo = totalAyahs ? `${ayahNumber}/${totalAyahs}` : ayahNumber;
        // Don't include URL in text - the Web Share API will add it from the url parameter
        const text = `📖 ${fullSurahName} - Ayah ${ayahInfo}\n\nRead the full surah on Quran Reader.`;

        // Check if Web Share API is supported
        if (navigator.share) {
            try {
                // Fetch the image as a blob to share it
                const response = await fetch(imageUrl);
                const blob = await response.blob();
                const file = new File([blob], `surah-${surahId}-ayah-${ayahNumber}.png`, { type: 'image/png' });

                // Try to share with file (image) - supported on some mobile browsers
                await navigator.share({
                    title: `${fullSurahName} - Ayah ${ayahInfo}`,
                    text: text,
                    url: ayahUrl,
                    files: [file]
                });
            } catch (err) {
                // If file sharing fails (not supported), try sharing without file
                if (err.name === 'NotSupportedError' || err.name === 'TypeError') {
                    try {
                        await navigator.share({
                            title: `${fullSurahName} - Ayah ${ayahInfo}`,
                            text: text,
                            url: ayahUrl
                        });
                    } catch (fallbackErr) {
                        console.error('Share failed:', fallbackErr);
                    }
                } else {
                    console.error('Share failed:', err);
                }
            }
        } else {
            // Fallback: Share to X (Twitter) - note: doesn't support images via intent
            shareToX();
        }
    };

    // Share to X (Twitter) - fallback for desktop without native share
    const shareToX = () => {
        const fullSurahName = surahEnglishName && surahTranslation
            ? `${surahEnglishName} (${surahTranslation})`
            : surahEnglishName || surahName;
        const ayahInfo = totalAyahs ? `${ayahNumber}/${totalAyahs}` : ayahNumber;
        const text = `📖 ${fullSurahName} - Ayah ${ayahInfo}\n\nRead the full surah on Quran Reader:`;
        const twitterUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(ayahUrl)}`;
        window.open(twitterUrl, '_blank', 'width=600,height=400');
    };

    // Share to WhatsApp - fallback for desktop without native share
    const shareToWhatsApp = () => {
        const fullSurahName = surahEnglishName && surahTranslation
            ? `${surahEnglishName} (${surahTranslation})`
            : surahEnglishName || surahName;
        const ayahInfo = totalAyahs ? `${ayahNumber}/${totalAyahs}` : ayahNumber;
        const text = `📖 ${fullSurahName} - Ayah ${ayahInfo}\n\nRead more on Quran Reader: ${ayahUrl}`;
        const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(text)}`;
        window.open(whatsappUrl, '_blank');
    };

    // Copy link to clipboard
    const copyLink = async () => {
        try {
            await navigator.clipboard.writeText(ayahUrl);
            setCopied(true);
            setTimeout(() => setCopied(false), 2000);
        } catch (err) {
            console.error('Failed to copy link:', err);
        }
    };

    // Download image
    const downloadImage = async () => {
        try {
            const response = await fetch(imageUrl);
            const blob = await response.blob();
            const url = window.URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            const suffix = imageFormat === 'portrait' ? '-story' : imageFormat === 'square' ? '-square' : '';
            a.download = `surah-${surahId}-ayah-${ayahNumber}${suffix}.png`;
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
            window.URL.revokeObjectURL(url);
        } catch (err) {
            console.error('Failed to download image:', err);
        }
    };

    // Get aspect ratio style based on format
    const getAspectRatio = () => {
        switch (imageFormat) {
            case 'portrait': return '9 / 16';
            case 'square': return '1 / 1';
            default: return '16 / 9';
        }
    };

    const modalContent = isOpen && (
        <div className="modal-overlay show" style={{ zIndex: 99999 }}>
            <div
                ref={modalRef}
                className="modal-content share-modal"
                style={{
                    marginTop: '20px',
                    boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)'
                }}
            >
                <div className="modal-header">
                    <h3 className="modal-title">Share Ayah {ayahNumber}</h3>
                    <button
                        className="btn-icon"
                        onClick={() => setIsOpen(false)}
                        aria-label="Close"
                    >
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                            <line x1="18" y1="6" x2="6" y2="18"></line>
                            <line x1="6" y1="6" x2="18" y2="18"></line>
                        </svg>
                    </button>
                </div>

                <div className="modal-body">
                    {/* Format Selection */}
                    <div className="format-selector" style={{ marginBottom: '16px' }}>
                        <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '8px', display: 'block' }}>
                            Image Format
                        </label>
                        <div style={{ display: 'flex', gap: '8px' }}>
                            <button
                                className={`btn btn-small ${imageFormat === 'landscape' ? 'btn-primary' : 'btn-secondary'}`}
                                onClick={() => setImageFormat('landscape')}
                                style={{ flex: 1, padding: '8px 12px', fontSize: '0.8rem' }}
                            >
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ marginRight: '4px' }}>
                                    <rect x="2" y="5" width="20" height="14" rx="2"></rect>
                                </svg>
                                Landscape
                            </button>
                            <button
                                className={`btn btn-small ${imageFormat === 'square' ? 'btn-primary' : 'btn-secondary'}`}
                                onClick={() => setImageFormat('square')}
                                style={{ flex: 1, padding: '8px 12px', fontSize: '0.8rem' }}
                            >
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ marginRight: '4px' }}>
                                    <rect x="3" y="3" width="18" height="18" rx="2"></rect>
                                </svg>
                                Square
                            </button>
                            <button
                                className={`btn btn-small ${imageFormat === 'portrait' ? 'btn-primary' : 'btn-secondary'}`}
                                onClick={() => setImageFormat('portrait')}
                                style={{ flex: 1, padding: '8px 12px', fontSize: '0.8rem' }}
                            >
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ marginRight: '4px' }}>
                                    <rect x="5" y="2" width="14" height="20" rx="2"></rect>
                                </svg>
                                Story
                            </button>
                        </div>
                    </div>

                    {/* Style Selection */}
                    <div className="style-selector" style={{ marginBottom: '16px' }}>
                        <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '8px', display: 'block' }}>
                            Image Style
                        </label>
                        <div style={{ display: 'flex', gap: '8px' }}>
                            <button
                                className={`btn btn-small ${imageStyle === 'classic' ? 'btn-primary' : 'btn-secondary'}`}
                                onClick={() => setImageStyle('classic')}
                                style={{ flex: 1, padding: '8px 12px', fontSize: '0.8rem' }}
                            >
                                Classic
                            </button>
                            <button
                                className={`btn btn-small ${imageStyle === 'nature' ? 'btn-primary' : 'btn-secondary'}`}
                                onClick={() => setImageStyle('nature')}
                                style={{ flex: 1, padding: '8px 12px', fontSize: '0.8rem' }}
                            >
                                Nature
                            </button>
                        </div>
                    </div>

                    {/* Image Preview */}
                    <div className="share-image-preview" style={{ marginBottom: '20px', minHeight: '180px' }}>
                        {imageLoading ? (
                            <div
                                style={{
                                    width: '100%',
                                    aspectRatio: getAspectRatio(),
                                    maxHeight: imageFormat === 'portrait' ? '300px' : 'auto',
                                    background: imageStyle === 'nature' ? '#1e293b' : 'linear-gradient(135deg, #fff7ed 0%, #ffedd5 100%)',
                                    borderRadius: '12px',
                                    display: 'flex',
                                    alignItems: 'center',
                                    justifyContent: 'center',
                                    color: imageStyle === 'nature' ? '#cbd5e1' : '#78716c',
                                }}
                            >
                                <div style={{ textAlign: 'center' }}>
                                    <div className="spinner" style={{ margin: '0 auto 8px', borderTopColor: imageStyle === 'nature' ? '#fff' : 'var(--accent-color)' }}></div>
                                    Generating...
                                </div>
                            </div>
                        ) : (
                            <img
                                src={imageUrl}
                                alt={`Ayah ${ayahNumber}`}
                                style={{
                                    width: '100%',
                                    maxHeight: imageFormat === 'portrait' ? '350px' : 'auto',
                                    objectFit: 'contain',
                                    borderRadius: '12px',
                                    boxShadow: '0 4px 20px rgba(0,0,0,0.15)',
                                    display: 'block'
                                }}
                            />
                        )}
                    </div>

                    {/* Share Options */}
                    <div className="share-options">
                        {/* Share on X - uses native share with image if supported, else falls back to intent */}
                        <button className="btn btn-secondary share-btn" onClick={nativeShare}>
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                            </svg>
                            Share on X
                        </button>
                        <button className="btn btn-secondary share-btn" onClick={copyLink}>
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                <rect x="9" y="9" width="13" height="2" rx="2" ry="2"></rect>
                                <path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path>
                            </svg>
                            {copied ? 'Copied!' : 'Copy Link'}
                        </button>
                        <button className="btn btn-primary share-btn" onClick={downloadImage}>
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                                <polyline points="7 10 12 15 17 10"></polyline>
                                <line x1="12" y1="15" x2="12" y2="3"></line>
                            </svg>
                            Download
                        </button>
                    </div>
                </div>
            </div>
        </div>
    );

    return (
        <>
            <button
                className="btn-icon"
                onClick={() => setIsOpen(true)}
                title="Share ayah"
            >
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <circle cx="18" cy="5" r="3"></circle>
                    <circle cx="6" cy="12" r="3"></circle>
                    <circle cx="18" cy="19" r="3"></circle>
                    <line x1="8.59" y1="13.51" x2="15.42" y2="17.49"></line>
                    <line x1="15.41" y1="6.51" x2="8.59" y2="10.49"></line>
                </svg>
            </button>
            {isOpen && createPortal(modalContent, document.body)}
        </>
    );
}

export default ShareButton;
